...
static int maxim_dsm_get_adaptive_apram(struct smart_amp_mod_struct_t *hspk,
					struct comp_dev *dev)
{
	enum DSM_API_MESSAGE retcode;
	int cmdblock[1 + SMART_AMP_FF_MAX_CH_NUM];
...
static inline void *_memmove(void *dest, const void *src, size_t count)
{
	char *tmp;
	const char *s;
			      ...
			      int nsamples, int szsample)
{
	...
	if (*w_ptr >= DSM_FF_BUF_SZ) {
		...
		if (remain) {
			if (is_16bit)
				_memmove(&buf.buf16[0],
					 &buf.buf16[DSM_FF_BUF_SZ],
					 remain * szsample);
			else
				_memmove(&buf.buf32[0],
	...
	if (*r_ptr >= nsamples) {
		...
		if (remain) {
			if (is_16bit)
				_memmove(&buf_out.buf16[0],
					 &buf_out.buf16[nsamples],
					 remain * szsample);
			else
				_memmove(&buf_out.buf32[0],
			      ...
			      int nsamples, int szsample)
{
	...
	if (*w_ptr >= DSM_FB_BUF_SZ) {
		...
		*w_ptr -= DSM_FB_BUF_SZ;
		...
		#ifdef PRINT_ADAPTIVE_LOG
		maxim_dsm_get_adaptive_apram(hspk, dev);
		#endif
	}
}
...
int smart_amp_get_memory_size(struct smart_amp_mod_struct_t *hspk, struct comp_dev *dev)
{
	enum DSM_API_MESSAGE retcode;
